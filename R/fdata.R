...
{	
    ...
    .fdata(y = y, N = N, r = r, S = S, 
          bycolumn = bycolumn, name = name, 
          type = type, sim = sim, exp = exp, 
          T = T)
}

setMethod("plot", signature(x = "fdata", 
                            y = "missing"), 
...
setReplaceMethod("setBycolumn", "fdata", 
                 function(object, value) 
                 {	 
...
setReplaceMethod("setName", "fdata", 
                 function(object, value) 
                 {
...
setReplaceMethod("setType", "fdata", 
                 function(object, value) 
                 {                     
        ...
        if (obj@bycolumn) {
            ...
            if (ncol(obj@S) > 1) {
                warning(paste("Wrong dimension of slot 'S' if ",
                              ...
                              "can only be one-dimensional.", sep = ""))
            }
        } else {
            if (!all(is.na(obj@y))) {                               
    ...
    if (!all(is.na(obj@T))) {
        ...
        if (any(obj@T < 1)) {
           stop(paste("Wrong specification of slot 'T'. Repetitions ",
                      "must be positive integers or NA.", sep = ""))
        }
...
".valid.type.Fdata" <- function(obj)
{
    if (!(obj@type %in% c("discrete", "continuous"))) {
        stop(paste("Wrong choice for slot 'type'. Data can be only ",
                   "'discrete' or 'continuous'", sep =""))
    }
}

### Valid r: The dimension of the data has tobe one for 'discrete' data
