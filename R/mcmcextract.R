...
setMethod( "moments", signature( object = "mcmcextract" ),
           function( object ) 
           {
               dist <- object@dist
               if ( dist == "normult" ) {
...
".moments.Normult.Mcmcextract"  <- function( obj ) 
{
    K   <- obj@K
    r   <- obj@r    
    moments.means   <- apply( sapply( seq( length = r ), 
    ...
    moments             <- list( mean = moments.means, var = moments.var, W = moments.W, 
