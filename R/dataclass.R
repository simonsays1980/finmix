...
"dataclass" <- function(fdata = NULL, model = NULL, simS = FALSE) 
{
    ...
    if (!model@indicfix) {
        if (model@indicmod == "multinomial") {				
            .multinomial.Dataclass(fdata, model, lik.list,
                                   simS)          
...
## Getters ##
setMethod("getLogpy", "dataclass", 
          function(object) 
          {
              return(object@logpy)
          }
)
setMethod("getProb", "dataclass", 
          function(object) 
          {
              return(object@prob)
          }
)
setMethod("getMixlik", "dataclass", 
          function(object) 
          {
              return(object@mixlik)
          }
)
setMethod("getEntropy", "dataclass", 
          function(object) 
          {
              return(object@entropy)
          }
)
setMethod("getLoglikcd", "dataclass", 
          function(object) 
          {
              return(object@loglikcd)
          }
)
setMethod("getPostS", "dataclass", 
          function(object) 
          {
              return(object@postS)
          }
...
".liklist.Dataclass" <- function(fdata.obj, model.obj)
{
    ...
    if(dist == "normal") {
        .likelihood.normal(datam, model.obj@par$mu, 
                                      model.obj@par$sigma)
    } else if(dist == "student") {
        .likelihood.student(datam, model.obj@par$mu,
                                       model.obj@par$sigma, 
                                       model.obj@par$df)
        ...
        model.obj   <- .check.Logdet.Norstud(model.obj)
        .likelihood.studmult(datam, model.obj@par$mu,
                             model.obj@par$sigmainv,
                             model.obj@par$logdet,
                             model.obj@par$df)
    } 
                                     ...
                                     lik.list, simS)
{
    ...
    if(simS) {
        datac.obj <- .dataclass(logpy = lik.list$llh, 
